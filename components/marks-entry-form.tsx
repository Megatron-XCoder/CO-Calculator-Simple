...
export default function MarksEntryForm({ examSetup }: MarksEntryFormProps) {
  ...
  // Group questions by CO for better organization
  const questionsByCO: Record<string, typeof examSetup.questions> = {}
  examSetup.questions.forEach((question) => {
    if (!questionsByCO[question.co]) {
      questionsByCO[question.co] = []
    }
    questionsByCO[question.co].push(question)
  })
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <Card className="border-blue-200 shadow-sm">
          ...
          <CardContent className="p-4 space-y-6">
            {Object.entries(questionsByCO).map(([co, questions]) => (
              <div key={co} className="space-y-3">
                <h3 className="font-semibold text-blue-700 border-b pb-1">{co}</h3>
